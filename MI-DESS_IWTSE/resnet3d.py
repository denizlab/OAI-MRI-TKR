...
class Resnet3DBuilder(object):
    ...
    def build(input_shape,num_outputs, block_fn, repetitions):
        ...
        input2 = Input(shape=(384, 384, 144, 1))
        
        filters = 32
        
        conv11 = _conv_bn_relu3D(filters=filters, kernel_size=(7, 7, 3),
                            strides=(2, 2, 1), name='Conv_{}_{}'.format(1, 1))(input1)
        pool12 = MaxPooling3D(pool_size=(3, 3, 3), strides=(2, 2, 1),
                             padding="same", name='Pool_{}_{}'.format(1, 1))(conv11)

        conv13 = _conv_bn_relu3D(filters=filters, kernel_size=(3, 3, 3),
                                strides=(2, 2, 2), name='Conv_{}_{}'.format(1, 2))(pool12)


        conv21 = _conv_bn_relu3D(filters=filters, kernel_size=(7, 7, 7),
                                strides=(2, 2, 2), name='Conv_{}_{}'.format(2, 1))(input2)
        pool22 = MaxPooling3D(pool_size=(3, 3, 3), strides=(2, 2, 2),
                             padding="same", name='Pool_{}_{}'.format(2, 1))(conv21)

        conv23 = _conv_bn_relu3D(filters=filters, kernel_size=(3, 3, 3),
                                strides=(2, 2, 2), name='Conv_{}_{}'.format(2, 2))(pool22)
...
#         padded23 = tf.pad(conv23, paddings, mode='CONSTANT',
#         constant_values=0.0)
        
        #padded23 = K.spatial_3d_padding(conv23, padding=((2, 2), (2, 2), (0, 0)), data_format="channels_last")
        #print(K.int_shape(padded23))
        #concatenated = concatenate([conv13, conv23],axis=-1)
        #concatenated = concatenate([conv13, conv23],axis=1)
        concatenated = add([conv13, conv23])
        #filters *= 1
        print(K.int_shape(concatenated))
        print('**********************')
        ...
        flatten1 = GlobalMaxPooling3D()(block_output)
        print(K.int_shape(flatten1))
        print('**********************')
        if num_outputs > 1:
